...
onMounted(() => {
  window.addEventListener('scroll', () => classHeader.value = window.scrollY ? 'bg-white text-gray-500' : '' );

  window.addEventListener('click', (e) => {
    if (!e.srcElement.classList.contains('img-person'))
      document.getElementById('menuRightSM').classList.add('hidden')
  })
})
</script>
<template>
  <div class="relative">
    <nav
      class="fixed left-0 top-0 z-50 w-full shadow-md"
      :class="classHeader ? classHeader : 'text-white'"
    >
      <!-- PC -->
        ...
        <div class="grid grid-cols-6 px-4 py-3 md:hidden">
              ...
              >
                <ul class="flex w-full flex-col items-start text-base text-gray-500">
                  ...
                  <li class="relative w-full px-6 py-4 text-left">
                    ...
                    <ul id="dropdownSM" class="left-0 mt-4 hidden">
                      <li>
          ...
          <div class="col-span-2 flex items-center justify-end text-center">
            <div v-if="isLogin" class="drop-sown-sm relative">
              <div class="flex items-center">
                <div class="me-2 text-sm"><b>6000</b> Scoin</div>
                <img
                  src="https://images.unsplash.com/photo-1535713875002-d1d0cf377fde"
                  alt="icon"
                  width="30"
