...
<template>
	<div class="fixed w-full">
		...
		<div class="px-4 py-4 flex text-center justify-center duration-700"
			:class="isCheckScroll ? 'bg-[#ffffffe6] text-gray' : 'bg-none text-white'">
			<nav class="w-[60vw] flex items-center justify-between">
				...
				<div>
					<ul class="hidden lg:flex lg:mx-auto lg:items-center lg:w-auto lg:space-x-6"
						:class="isCheckScroll ? 'bg-[#ffffffe6] text-gray-500' : 'text-white'">
						<router-link v-for="item in menu" :key="item" :to="item.url"
							class="text-sm font-medium hover:text-[#ffff00] hover:shadow-yellow-300 hover:shadow-lg">
							{{ item.name }}
						</router-link>
